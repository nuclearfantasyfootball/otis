...
#'
#' @seealso \code{\link{get_league_users}} for the raw user data before expansion.
#'
#' @examples
#' \dontrun{
...
league_users_details <- function(league_id) {
  ...
  users_detailed <- users |>
    dplyr::select(
      ...
      is_bot, is_owner,
      avatar, # Used to get user avatar, not team
...
# Run code if not package build/install with league_id
if (interactive()) {
  # Look at sleeper url - https://sleeper.com/leagues/<league_id>/matchup
  league_id = "<league_id>"
  users_raw <- get_league_users(league_id) # users api raw data
  users <- league_users_details(league_id) # users parsed data
}
